...
# Prog Purpose: This program reads a comma-delimited data file inkto a LIST of smaller lists.
#   The dat for each student is appended to a list with six data fields:
#   Postion 0 is last name, 1 is first name, 2 is homework, 3 is Midterm exam, 4 is Final exam
...
def read_data_file():
    ...
    for i in lines:
        stu.append(i.split(",")) #Split the data by comma and add each section to the list
    datafile.close()        
...
def calculate_grades():
    ...
    for i in range(len(stu)):
        hw = int(stu[i][2]) # Last name is pos. 0, first name is pos. 1, so homework is ipos. 2
...
def create_report_file():
    ...
    repfile.write("\nTotal numver of students: " + str(len(stu)))
